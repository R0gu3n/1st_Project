...
.then(res =>{
  res.json().then(data =>{
    for(var i=0; i<100; i++){
      ...
      if(change < 0){
        $("#tableBody1").append("<tr><td>"+ rank + "</td>"+ "<td><img src= "+ pic +" style= 'width:20px; height:auto;' class= 'image' />"+ coin + "</td>"+
...
/*$("#tableBody1").append("<tr><td>"+ rank + "</td>"+ "<td><img src= "+ pic +" style= 'width:25px; height:auto;' />"+ coin + "</td>"+
   "<td>"+ mktcap + "</td>"+ "<td>"+ price + "</td>"+ "<td>"+ volume + "</td>"+
   "<td>"+ supply + "</td>"+ "<td class='red'>"+ change +  "</td></tr>");*/
