...
var coinDataURL = "/coins/markets?vs_currency=usd&order=market_cap_desc%2C%20volume_desc%2C%20id_desc&per_page=200&page=1&sparkline=false&price_change_percentage=24h";
...
.then(res =>{
  res.json().then(data =>{
    for(var i=100; i<200; i++){
      ...
      if(change < 0){
        $("#tableBody1Next").append("<tr><td>"+ rank + "</td>"+ "<td><img class= 'image' src= "+ pic +"/>"+ coin + "</td>"+
